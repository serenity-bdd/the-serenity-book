...
To do its reporting magic, Serenity BDD needs to instrument the task and action objects used during the tests. The simplest way to arrange this is to let Serenity create it for you, just like any other Serenity step library, using the `@Steps` annotation. In the following code snippet, Serenity will instantiate the `openTheApplication` field for you, so that James can use it to open the application:
...
However, hard-coding the CSS selector could lead to duplication. A better practice would be to refactor the selector into a simple _Page Object_ class, like this one:
